...
class Export extends MY_Controller {
							...
							foreach ($get_fkpbj as $key_fk => $value_fk) {
						        ...
						        // echo 'Ini yellow start '.$yellow_start;
						       	...
						       	if ($entry_date > $yellow_end_) {
									...
									// echo $fkpbj;
									$week__ 		= $this->date_week_actual($year, $value_fk['entry_stamp'], $yellow_start);
						       	} else {
						       		...
						       		$jwpp = json_encode(array('start'=>$entry_stamp,'end'=>$jwpp_end));
									$week__ 		= $this->date_week_actual($year, $entry_stamp, $yellow_start,$metode,$jwpp);
						       	}
								...
								// }
								$table 		.= '<tr class="content" style="border-bottom:0.01em solid black;">
												<td id="'.$value_fk['id_fppbj'].'"></td>
												<td>'.$value_fk['divisi'].'</td>
												<td>'.$value_fk['nama_pengadaan'].'</td>
												<td>'.$value_fk['metode_pengadaan_name'].'</td>
												<td>Rp. '.number_format($value_fk['idr_anggaran']).'</td>
												<td>'.$jenis_pengadaan.'</td>
												<td>'.$fkpbj.'</td>
												'.$week__.'
											</tr>
	...
	function date_week_actual($year=null, $fkpbj, $yellow_end, $metode,$jwpp){
		define('NL', "\n");
		$jwpp 			= json_decode($jwpp);
		// echo $metode;die;
		...
		if ($fkpbj > $yellow_end) {
			...
			$end_yellow		= $end_red ;
			// $start_yellow 	= $fkpbj;
			// $start_red		= $start_yellow + $metode_day;
			// $end_yellow		= $start_red;
			// $end_red		= $start_red;
		}else{
			// echo "Kon 2";die;
			//Variable 
			// $start			= $this->get_week($fkpbj);
			// $end			= $this->get_week($fkpbj);
			$start_red		= $end_yellow + 1;
			$end_red		= $start_red + $metode_day;
			$start_yellow	= $this->get_week($fkpbj);
			$end_yellow		= $start_yellow + 1 ;
		}
